...
<body>
	<div class="well">
		<h1 style="color:#34495E; font-family:times new roman,sans serif; text-align:center; margin-top:40px; font-size:40px;">Welcome To Indirapuram</h1><br>
    </div>    
		
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" style="margin-top=50px"  method="GET">
	<select name="foods">
	...
	<input type="submit" style="background-color:#273746; color:white; box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19); margin-left:75px; padding:5px 10px;">
        ...
        $sql='select * from pg where locality="Indirapuram"';
        ...
        if(mysqli_num_rows($result)>0)
        {
			...
			while($row=mysqli_fetch_assoc($result))
            {
				...
				else if($acs==$row["airconditioned"]  && $foods==$row["food"] && $range==2 && $row["rent"] > 5000 && $row["rent"] <= 10000)
				{
				...
				else if($acs==$row["airconditioned"]  && $foods==$row["food"] && $range==3 && $row["rent"] > 10000 && $row["rent"] <= 15000)
				{
